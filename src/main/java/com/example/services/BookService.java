...
/**
 * The {@code BookService} provides high-level business logic for managing the library catalog 
 * and circulation processes. It acts as the primary interface between the UI and the 
 * persistence layer ({@link BooksDB}).
 *
 * <p>Core Responsibilities:
 * <ul>
 *   <li><b>Catalog Management:</b> Validating and persisting new book entries or modifications.</li>
 *   <li><b>Circulation Logic:</b> Orchestrating book issuance, returns, and ensuring compliance 
 *       with library policies (e.g., borrow limits, loan periods).</li>
 *   <li><b>Reporting & Analytics:</b> Generating statistics and overdue tracking across the entire collection.</li>
 * </ul>
 *
 * <p>This service is thread-safe and utilizes centralized logging to track all major 
 * library transactions.</p>
 *
 * @see Book
 * @see BooksDB
 * @see BorrowRequestService
 */
public final class BookService {

    private static final Logger LOGGER = Logger.getLogger(BookService.class.getName());

    private static BooksDB booksDB() { return BooksDB.getInstance(); }
    private static UsersDB usersDB() { return UsersDB.getInstance(); }

    // Business rule constants
    /** Minimum allowed length for a book title. */
    private static final int MIN_TITLE_LENGTH = 1;
    /** Maximum allowed length for a book title. */
    private static final int MAX_TITLE_LENGTH = 200;
    /** Minimum allowed length for an author's name. */
    private static final int MIN_AUTHOR_LENGTH = 1;
    /** Maximum allowed length for an author's name. */
    ...
    /** Minimum allowed length for an ISBN (standard ISBN-10). */
    private static final int MIN_ISBN_LENGTH = 10;
    /** Maximum allowed length for an ISBN (standard ISBN-13 + padding). */
    ...
    /**
     * Issues copies of a book to a user. This is the primary entry point for the library 
     * circulation workflow.
     *
     * <p>This method performs several critical checks before completing the transaction:
     * <ol>
     *   <li>Verifies that both the book (by ISBN) and user exist.</li>
     *   <li>Ensures the library has enough physical copies in stock.</li>
     *   <li>Validates that the user hasn't exceeded their maximum allowed borrow limit.</li>
     * </ol>
     *
     * @param isbn the ISBN of the book to issue
     * @param userId the ID of the borrowing user
     * @param quantity the number of copies to issue
     * @throws BooksException if stock is insufficient or policy limits are exceeded
     * @throws UserException if the user record cannot be found
     */
    ...
    public static Map<String, Object> getLibraryStatistics() {
        try {
            ...
            // FIX: Track returned books that still have outstanding settlement fines.
            // When a book is returned via "Pay Later" it is no longer "overdue" (correctly),
            // but the unpaid fine is visible only in the Settlements tab — the old dashboard
            // never exposed this number so it looked like the fine had disappeared.
            List<IssueRecord> allRecords = getAllIssueRecords();
            long outstandingSettlements = allRecords.stream()
                    .filter(r -> r.isReturned() && r.getRemainingFine() > 0)
                    .count();
            double totalSettlementFines = allRecords.stream()
                    .filter(r -> r.isReturned() && r.getRemainingFine() > 0)
                    .mapToDouble(IssueRecord::getRemainingFine)
                    .sum();
            ...
            statistics.put("outstandingSettlements", (int) outstandingSettlements);
            statistics.put("totalSettlementFines", totalSettlementFines);
    ...
    public static void updateLibraryConfiguration(int maxBorrowLimit, int loanDays, double finePerDay)
        ...
        if (maxBorrowLimit < 1 || maxBorrowLimit > 100) {
            throw new BooksException("Max borrow limit must be between 1 and 100.");
        }
        if (loanDays < 1 || loanDays > 365) {
            throw new BooksException("Loan period must be between 1 and 365 days.");
        }
        if (finePerDay < 0) {
            throw new BooksException("Fine amount cannot be negative.");
        }
    ...
    public static void seedSampleData() {
        ...
        for (String[] b : books) {
            try {
                ...
                LOGGER.log(Level.WARNING, "Failed to import book row ISBN=" + b[0]
                        + " — row skipped: " + e.getMessage(), e);
            }
        }
        booksDB().saveAllData();
    }
